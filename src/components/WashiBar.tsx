...
export function WashiBar({
  ...
  const left = initiative.startMonth * monthWidth;
  const width = (initiative.endMonth - initiative.startMonth + 1) * monthWidth;
  ...
  return (
    <div className="absolute" style={{ left, top, width, height: barHeight, zIndex: 4 }}>
      <div
        style={{ position: 'absolute', left: 0, top: 0, width: handleWidth, height: '100%', cursor: 'ew-resize', zIndex: 6, borderRadius: '4px 0 0 4px' }}
      ...
      >
        <span style={{ fontSize: 11.5, fontWeight: 500, fontFamily: '"DM Sans", sans-serif', color: initiative.textColor, letterSpacing: '0.01em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', pointerEvents: 'none', userSelect: 'none' }}>
          {initiative.title}
        </span>
      </div>

      <div
        style={{ position: 'absolute', right: 0, top: 0, width: handleWidth, height: '100%', cursor: 'ew-resize', zIndex: 6, borderRadius: '0 4px 4px 0' }}
